...
static void Init(void)
{
    Chip_Clock_System_SetClockFreq(4 * 1000 * 1000);

	while( SysTick_Config(4 * 1000 * SYSTICK_INTERVAL) )
	{
		;
	}
	...
	Chip_TIMER_PrescaleSet(LPC_TIMER16_0, 4000000);// PR   1000hz
	Chip_TIMER_SetMatch(LPC_TIMER16_0, 0, 1000000);// MR(x)  1s
...
int System_ClockDiv, System_ClockFreq;
int main(void)
{
	Init();
	...
	while(1)
	{
		while_test_cnt = LPC_TIMER16_0->PC;
	}
	//return 0;
}
